      ...
      // Functions to manage position-specific metric selections
  ...
  pizzaChartInstance = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            borderColor: typeof window.isDarkMode === 'function' && window.isDarkMode() ? '#1a1a1a' : '#ffffff',
            borderWidth: 2,
        }]
    },
    options: {
        plugins: {
            ...
            datalabels: {
                ...
                color: function(context) {
                    // Use white in dark mode, segment colors in light mode
                    if (typeof window.isDarkMode === 'function' && window.isDarkMode()) {
                        return '#ffffff';
                    }
                    // Return the corresponding segment color in light mode
            ...
            beforeDraw: (chart) => {
                ...
                // Draw the outer circle with adaptive color for dark mode
                const currentColors = typeof window.getChartColors === 'function' ? 
                    window.getChartColors() : { gridColor: 'rgba(0, 0, 0, 0.5)' };
                
                ctx.save();
                ctx.beginPath();
                ctx.arc(centerX, centerY, radius, 0, 2 * Math.PI);
                ctx.lineWidth = 2;
                ctx.strokeStyle = currentColors.gridColor;
                ctx.stroke();
                ctx.restore();
            }
        },
...
document.getElementById('chartButton').innerHTML = `
       <div class="dropdown">
            <button class="dropbtn"><i class="fa fa-pencil"></i></button>
            <span class="chart-tooltip" data-i18n="buttons.edit_graphs">Edit graphs</span>
            <div id="metric-controls" class="dropdown-content">
                <label style="border-bottom: 1px solid var(--border-color); margin-bottom: 8px; padding-bottom: 8px;">
                    <input type="checkbox" id="chartSortingToggle" ${sortEnabled ? 'checked' : ''}>
                    <span class="checkbox-label-text" data-i18n="buttons.show_ascending_ranks">Show ascending ranks</span>
                </label>
                <!-- Other checkboxes will be generated here by JavaScript -->
            </div>
</div> `;
...
function populateMetricControls() {
    ...
    // Remove only metric checkboxes, preserve sorting checkbox
    const metricLabels = metricControlsDiv.querySelectorAll('label:not([style*="border-bottom"])');
    metricLabels.forEach(label => label.remove());
    
    // Add event listener for sorting checkbox
    const sortingCheckbox = document.getElementById('chartSortingToggle');
    if (sortingCheckbox && !sortingCheckbox.dataset.listenerAdded) {
        sortingCheckbox.dataset.listenerAdded = 'true';
        sortingCheckbox.addEventListener('change', function() {
...
// Updated populateMetricControls function for first position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
    ...
    // Apply stored metrics for this position
    ...
    Object.keys(metricsToInclude).forEach(metric => {
        ...
        checkbox.addEventListener('change', function() {
            ...
            // Save current metrics for this position
...
// Updated populateMetricControls function for second position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
// Updated populateMetricControls function for third position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
document.getElementById('chartButton').innerHTML = `
    ...
    if (typeof window.translateNewElements === "function") window.translateNewElements();    createPizzaChart(rankData, player.player);
// JavaScript part
let userExclusions = new Set();
...
// Updated populateMetricControls function for fourth position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
// Updated populateMetricControls function for fifth position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
// Updated populateMetricControls function for sixth position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
// Updated populateMetricControls function for seventh position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
// Updated populateMetricControls function for eighth position
function populateMetricControls() {
    const metricControlsDiv = document.getElementById('metric-controls');
...
function populatePlayerOptions() {
    ...
    // --- FIX: Populate the hidden playerSelect dropdown with all players ---
    const playerSelect = document.getElementById('playerSelect');
    ...
    // Generate search variations for enhanced matching
    ...
    function selectPlayer(player) {
        // Find the player option in the hidden dropdown and select it
        const playerSelect = document.getElementById('playerSelect');
    ...
    // Global keydown handler for search autocomplete (matching custom selector style)
    document.addEventListener('keydown', function(e) {
        // Only process when autocomplete list is open and visible
...
// Enhanced search functionality with the same smart matching
...
searchInput.addEventListener('input', function () {
        ...
        if (searchText === '') {
            ...
            for (let i = 0; i < playerSelect.options.length; i++) {
                ...
                if (i === 0) {
                    ...
                    // Check if any search variation matches the option text
                    let matches = false;
  ...
  // Simply toggle the active state of the clicked element
  // This allows multiple tooltips to be shown simultaneously
