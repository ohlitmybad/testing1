...
// Helper function to create unique player ID
function getPlayerUniqueId(d) {
    return d[0] + '-' + d[1]; // ID + Player name
}

// Helper function to get player name from unique ID
function getPlayerNameFromId(uniqueId) {
    const parts = uniqueId.split('-');
    return parts.slice(1).join('-'); // Everything after the first dash is the name
}

// Helper function to get player ID from unique ID
...
.then(workerResponse => {
    ...
    fileAliases.forEach((alias) => {
        ...
        if (alias === firstFileAlias && !isFirstFileProcessed) {
        ...
        // 5. Collect the transformed data (excluding header for non-first file)
        allData.push(...(alias === firstFileAlias ? jsonData : jsonData.slice(1)));
    });
...
.catch(error => {
    ...
    if (chartContainer) {
        chartContainer.innerHTML = '<p style="color: red; padding: 20px;">Error loading data. Please try again later.</p>';
    }
}); // Close the worker fetch promise chain
